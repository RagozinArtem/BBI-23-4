...
class task15 : Zadanie
{
    ...
    {
        _sum = 0;
        _words = _text.Split(' ', '.', '!', ';', ':', '?', '(', '),'а', 'б', 'в', 'г', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'ъ', 'ы', 'ь', 'э', 'ю', 'я'');
    }
    ...
    public override string ToString()
    {
        ...
        foreach (string word in _words)
        {
            ...
            _sum += Convert.ToDouble(word);
        }
        Console.WriteLine(Math.Round(_sum, 2));
        return base.ToString();
    }
}
...
class Program
{
    public static void Main()
    {
        ...
        task15 fifteen = new task15("4 6,5 -7 -9,12");
        sixth.ToString();
        first.ToString();
        third.ToString();
        thirtinth.ToString();
